resource "aws_instance"  "ec2-Terraform-Vpc" {
count = length(var.EC2_names)
ami = data.aws_ami.amazon-2.id
instance_type = "t2.micro"
vpc_security_group_ids = [var.aws_security_group-terraform ]
associate_public_ip_address = true
subnet_id = var.aws_subnet_id_from_vpc_module[count.index]
availability_zone = data.aws_availability_zone.Terraform-availability_zone-EC2.names[count.index]
user_data = <<EOF
  #!/bin/bash
    sudo yum update -y
